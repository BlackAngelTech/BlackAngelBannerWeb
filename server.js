...
const ACTIVE_SESSIONS_FILE = path.join(__dirname, 'connectedUsers.json'); // from your bot
...
app.post('/api/login', (req, res) => {
    ...
    // Check admin default
    if (username === 'admin' && password === 'BlackAngel123') {
...
app.get('/api/admin/users', isAdmin, (req, res) => {
    ...
    const enriched = users.users.map(u => ({
        ...
        active: !!activeSessions[u.username] // from connectedUsers.json
    }));
    res.json(enriched);
});
...
app.post('/api/admin/premium', isAdmin, (req, res) => {
    const { username, premium } = req.body; // premium is boolean
    const users = readJSON(USERS_FILE);
    const user = users.users.find(u => u.username === username);
    if (!user) return res.json({ success: false, message: 'User not found' });
...
// Get active sessions (from connectedUsers.json)
